...
- Task 24: i18n — `next-intl` with four locales, `localePrefix: 'as-needed'`, all routes under `[locale]`, every string extracted, typed keys via `createMessagesDeclaration`, locale/auth composition in `proxy.ts`, and a `satisfies`-based parity gate riding `tsc`
